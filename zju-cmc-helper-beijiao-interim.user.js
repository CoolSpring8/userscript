// ==UserScript==
// @name         浙江大学智云课堂小助手 for 北教智云（临时）
// @description  北教智云课堂功能增强（临时解决方案）
...
class CmcHelper {
  constructor() {
    ...
    const _init = () => {
      ...
      const courseElem = querySelector(".container")
      const playerElem = querySelector(".container")

      if (
        !this._isVueReady(courseElem) ||
        !this._isVueReady(playerElem) ||
        !("CmcPCPlayer" in myWindow || "CmcMobilePlayer" in myWindow)
      ) {
      ...
      const originalToolbar = querySelector(".help-wrapper")
  ...
  async downloadPPTImages() {
    ...
    const blob = await downloadZip(
      this._batchFetch(
        pptList.map((ppt) => ({
          url: ppt.pptimgurl.replace(/^http:/, "https:"),
          info: { timeInVideo: ppt.created_sec },
        })),
        (resp, url, info) => {
          ...
          return {
            ...
            name: `${p[0].value}-${p[2].value}-${p[4].value}_${p[6].value}-${
              p[8].value
            }-${p[10].value}__${this._secondsToDuration(
              info.timeInVideo
            )}.${ext}`,
          }
        }
  ...
  exportSpeechText() {
    window.open(
      URL.createObjectURL(
        new File(
          [
            [...document.querySelectorAll(".trans-lan")]
